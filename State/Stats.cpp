...
void Stats::formatVideoStats(DX::StepTimer const& timer, VIDEO_STATS& stats, char* output, size_t length) {
	...
	if (stats.receivedFps > 0) {
		...
		switch (Pacer::instance().getPacingMode()) {
			...
			case Pacer::PACING_ADAPTIVE:       pacingModeStr = "adaptive";       break;
			default:                           pacingModeStr = "immediate";      break;
		}
		...
		if (Pacer::instance().getPacingMode() == Pacer::PACING_ADAPTIVE) {
			ret = snprintf(&output[offset], length - offset,
			               "Adaptive: target %d (hwm %d)  decode_max %.2f ms  jitter %.2f ms  phase_min %.2f ms  burst %.1f  [logging: %s]\n",
			               ...
			               Pacer::instance().getPhaseMarginMinMs(), Pacer::instance().getArrivalBurstScore(),
			               isCsvLogging() ? "ON" : "off");
			if (ret > 0 && (size_t)ret < (length - offset)) {
				offset += ret;
			}
...
void Stats::logCsvLine(VIDEO_STATS& s, double now) {
	...
	int n = snprintf(buf, sizeof(buf),
	                 "%.1f,%s,%.2f,%.2f,%.2f,%.2f,%.3f,%.3f,%.3f,%.3f,%.3f,%.3f,%.3f,%u,%u,%u,%u,%.1f,%.3f,%.3f,%.3f,%.2f,%d,%d,%.3f,%.2f\n",
	...
	try {
		...
		if (f.is_open()) {
			f << "t_s,mode,recv_fps,dec_fps,rend_fps,frames_in_q,q_ms,render_ms,present_ms,"
			     ...
			     "ft_mean_ms,ft_sd_ms,ft_max_ms,missed_pct,adaptive_target,hwm,phase_min_ms,burst_score\n";
		}
	} catch (...) {
		m_csvPath.clear();
		return;
